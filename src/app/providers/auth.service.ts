...
>>>>>>> b6c4a4b9d83b8fdc086b5999411adc835e136715
    ...
    authenticationToken(
        ...
        // tslint:disable-next-line: max-line-length
        return this.http.get(
            `${this.localConfig.authenticationUrl}?username=${username}&password=${password}&request_token=${requst_token}&api_key=${this.localConfig.apiKey}`
        );
    }

    getSession(requst_token: string) {
        return this.http.get(
            `${this.localConfig.sessionUrl}/new?api_key=${this.localConfig.apiKey}&request_token=${requst_token}`
        );
    }

    getUserData(session_id: string) {
        return this.http.get(
            `${this.localConfig.accountUrl}?api_key=${this.localConfig.apiKey}&session_id=${session_id}`
        );
    }
    ...
    removeSession(httpParams) {
        ...
        return this.http.delete(
            `${this.localConfig.sessionUrl}?api_key=${this.localConfig.apiKey}`,
            httpOptions
        );
    }
